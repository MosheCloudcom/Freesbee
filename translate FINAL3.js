// Add a CSS style directly to the script tag
document.currentScript.style.textAlign = 'right';
...
function translate() {
    ...
    if (subtitleElement) {
        ...
        subtitleElement.innerText = 'ניתן למלא כתובת אימייל לקבלת קישור להמשך מילוי ושליחת הטופס';
    } else {
    ...
    if (copyLinkTitleElement) {
        ...
        copyLinkTitleElement.innerText = "לחץ להעתקת הקישור להמשך מילוי ושליחת הטופס";
    } else {
...
// Select the span element within the button
 // Select the button element by its class
        var buttonElement = document.querySelector('.save-for-later-page__btn--secondary');

        buttonElement.textContent = 'העתק קישור';

   // Select the email sent element by its class
    var emailSent= document.querySelector('.save-for-later-page__saved-msg');

    // Change the text content of the email sent element
       emailSent.innerText = 'האימייל נשלח בהצלחה עם קישור להמשך  מילוי ושליחת הטופס';
}
